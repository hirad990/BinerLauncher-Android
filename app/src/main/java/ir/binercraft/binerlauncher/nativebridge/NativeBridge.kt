...
object NativeBridge {
    init { System.loadLibrary("binerlauncher-native") }

    external fun nativeGetEngineInfo(): String
    external fun nativeAbiVersion(): Int
    external fun nativeAttachSurface(surface: Surface): Boolean
    external fun nativeDetachSurface()
    external fun nativeRenderTestFrame(timeSeconds: Float)
    ...
    fun attachSurface(surface: Surface): Boolean = nativeAttachSurface(surface)
    fun detachSurface() = nativeDetachSurface()
    fun renderTestFrame(timeSeconds: Float) = nativeRenderTestFrame(timeSeconds)
